...
pub fn init_submodule(repo_at: &str, wit_dir: Option<&str>, from: &str) {
    ...
    // 1. In .gitmodules
    {
        let mut gitmodules_file = std::fs::OpenOptions::new()
            .read(true)
            .open(git_repo.workdir().unwrap().join(".gitmodules")).unwrap();
        ...
        let to_modify = lines.iter_mut().find(|line| line.contains(&pattern_to_replace)).unwrap();
    ...
    // 2. In .git/config
    {
        ...
        config.set_str(&(new_key_prefix.clone() + ".url"), &old_url_val).unwrap();        
        config.set_str(&(new_key_prefix + ".path"),&wit_dir).unwrap();
}
...
pub fn add_files(repo_at: &str, files: &[&str]) {
    ...
    index.add_all(files, git2::IndexAddOption::DEFAULT, None).unwrap();
    index.write().unwrap();
}
...
pub fn commit(repo_at: &str, msg: &str) {
    ...
    // Prepare commit
    let signature = submodule_repo.signature().unwrap_or_else(|_| {
    ...
    let previous_commit_oid = submodule_repo.resolve_reference_from_short_name("HEAD").unwrap().peel_to_commit().unwrap();
    
    // Create a commit within the submodule
    submodule_repo.commit(Some("HEAD"), &signature, &signature, msg, &commit_tree, &[&previous_commit_oid]).unwrap();
...
pub fn push(repo_at: &str) {
    ...
    remote.push(&["refs/heads/master:refs/remotes/origin/master"], None).unwrap();
}
